...
class task_5 {
    public static void main(String[] args) {
        ...
        System.out.println(timeDifference("London", "July 21, 1983 23:01", "Rome"));
    }
    ...
    public static String spiderVsFly(String spider, String fly) {
        ...
        int k = (Math.abs(fx-sx)<4?Math.abs(fx-sx):8 - Math.abs(fx-sx));
        double strategyDist2 = Math.abs(sy - fy) + k * Math.min(sy,fy) * 0.76536686473;
        ...
        if (strategyDist1 <= strategyDist2) {
            ...
            if(fy > sy){
                ...
                //ring
                for (int i = 0; i <= 8 - Math.abs(fx - sx); i++) {
                    path += (char)(65 + (sx + i) % 8);
                    path += fly.charAt(1);
                    if (path.charAt(path.length()-2) == fx+65){
                        path += fly.charAt(1);
                        break;
                    }
                    path += '-';
                ...
                if(path.length()>12){
                    ...
                    for (int i = 0; i <= Math.abs(fx - sx) ; i++) {
                        path += (char)(65 + (sx - i) % 8);
                        path += fly.charAt(1);
                        path += '-';
                    }
    ...
    public static int totalPoints(String[] words, String scramble) {
        int points = 0;
        int[] scrambleCharset = Helper.getCharset(scramble);
        for (int i = 0; i < words.length; i++) {
            int[] wordCharset = Helper.getCharset(words[i]);
            boolean good = true;
            for (int j = 0; j < 127; j++)
    ...
    public static String takeDownAverage(String[] percents) {
        ...
        double res = ((sum/percents.length)-5) * (percents.length+1) - sum;
        return (int)res + "%";
    }
    ...
    public static int maxPossible(int n, int r) {
        Integer[] num = Helper.splitNumber(n);
        Integer[] rnum = Helper.splitNumber(r);
